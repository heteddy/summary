...
- **`const`** **const** - Makes the mocked method a `const` method. Required if  
  overriding a `const` method.
- **`override`** **override** - Marks the method with `override`. Recommended if overriding  
  a `virtual` method.
- **`noexcept`** **noexcept** - Marks the method with `noexcept`. Required if overriding a  
  `noexcept` method.
- **`Calltype(...)`** **Calltype(...)** - Sets the call type for the method (e.g. to  
...
recipe for [mocking non-virtual methods](for_dummies.md#MockingNonVirtualMethods).
...
[Understanding Uninteresting vs Unexpected Calls](for_dummies.md#uninteresting-vs-unexpected).
...
2. `NiceMock<MockFoo>` and `StrictMock<MockFoo>` may not work correctly if the  
   ...
   requires cleaning up existing tests. [http://b/28934720](http://b/28934720) tracks the issue.
...
- The general technique discussed here works for overloaded methods, but  
  ...
  use [this technique](for_dummies.md#SelectOverload); to disambiguate a fake function (the  
...
This technique is very similar to the [delegating-to-fake](for_dummies.md#DelegatingToFake)  
...
including the `Lt()` matcher above. See [here](for_dummies.md#MultiArgMatchers) for the  
complete list.
...
gMock provides a [built-in set](for_dummies.md#MatcherList) of matchers. In case you find them  
...
that there are [AllOf()](for_dummies.md#CombiningMatchers)[AllOf()](for_dummies.md#CombiningMatchers)[ and ](for_dummies.md#CombiningMatchers)[AllOfArray()](for_dummies.md#CombiningMatchers)[AllOfArray()](for_dummies.md#CombiningMatchers).
...
messages, you should [define a matcher](for_dummies.md#NewMatchers). If you want to do it  
...
**`ON_CALL`** **ON_CALL** is likely the *single most under-utilized construct* in gMock.
...
`DefaultValue<T>::Set()` (described [here](for_dummies.md#DefaultValue)) or `ON_CALL()`.
...
["The Nice, the Strict, and the Naggy"](for_dummies.md#NiceStrictNaggy).
...
supplying a [simpler mock interface](for_dummies.md#SimplerInterfaces) than the mocked class  
provides.

This pattern is also useful when the arguments are interesting, but match logic  
is substantially complex. You can leave the argument list unspecified and use  
SaveArg actions to [save the values for later verification](for_dummies.md#SaveArgVerify). If  
...
calls. One way to express the DAG is to use the [After](for_dummies.md#AfterClause) clause of  
`EXPECT_CALL`.
...
The [selecting-an-action's-arguments](for_dummies.md#SelectingArgs) recipe showed us one way  
...
[workarounds](for_dummies.md#LegacyMoveOnly) for lack of this feature.
...
[Setting Default Actions](for_dummies.md#OnCall).
...
can always use `Return`, or a [lambda or functor](for_dummies.md#FunctionsAsActions):
...
that you can specify constraints on the [order](for_dummies.md#OrderedCalls) of mock function  
...
[this section](for_dummies.md#PureMatchers) for more details.
...
of several types (for instance, `Eq(x)` can be used to match a *`value`* *value* as long  
as `value == x` compiles -- *`value`* *value* and `x` don't have to share the same  
...
If the [built-in set](for_dummies.md#CardinalityList) of cardinalities doesn't  
...
[googletest's advanced guide](for_dummies.md#teaching-googletest-how-to-print-your-values)  
