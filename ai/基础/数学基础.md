...
机器学习领域，我们有一个步长（记为  ，发音 gamma 通常取值在 0 - 1 之间）是人为设定的值，它与梯度（记为 ，发音 grad）相乘，就可以描述小兔子每蹦跶一下该往某个方向跳以及一下跳了多远了。
...
刚才的例子也说了变化率描述山路的陡峭程度，放在函数里，描述函数变化率的东东就是函数的**导数**。在一元函数里，我们的函数就是从左到右的一根线，方向已经固定了，没有东南西北可言，所以**对于一元函数而言，函数****在** **点处的梯度，等于该函数的导数在****点处的值，即**，**记为**。说白了就是方向上没得选，于是梯度直接等于导数。
