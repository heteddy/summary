...
- 网络退化问题(degradation problem)
  ...
  构建深层神经网络时，在网络可以收敛的情况下，随着网络层数的增加，网络性能先增加后迅速下降，这并不符合常理，因为更深的网络结构通常被认为应该表现更好；

  例如一个56层的神经网络在测试集错误率会高于一个20层的浅层网络，且并非数据问题导致，即使在训练集上，深层网络的表现也不如浅层网络这种现象就叫做**网络退化**

![](.images/tbY2Yub4cOS3g.jpeg)
...
- residual mapping，指的就是除了”弯弯的曲线“那部分，所以最后的输出是 y=F(x)+x
