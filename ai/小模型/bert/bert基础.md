...
传统的语言模型训练, 都是采用left-to-right, 或者left-to-right + right-to-left结合的方式, 但这种单向方式或者拼接的方式提取特征的能力有限. 为此BERT提出一个深度双向表达模型(deep bidirectional representation). 即同时利用正向预测和反向预测的优势，从两个方向阅读句子即从左到右和从右到左，双向模型能给出更好的结果，因为从两个方向阅读句子，模型能更加清晰的理解句子
