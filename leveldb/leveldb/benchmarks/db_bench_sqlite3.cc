                                ...
                                if(name == Slice("overwrite")) {
                                    ...
                                    if(name == Slice("overwritebatch")) {
                                        ...
                                        if(name == Slice("fillrandsync")) {
                                            ...
                                            if(name == Slice("fillseqsync")) {
                                                write_sync = true;
                                                Write(write_sync, SEQUENTIAL, FRESH, num_ / 100, FLAGS_value_size, 1);
                                                WalCheckpoint(db_);
                                            } else
                                                if(name == Slice("fillrand100K")) {
                                                    Write(write_sync, RANDOM, FRESH, num_ / 1000, 100 * 1000, 1);
                                                    WalCheckpoint(db_);
                                                } else
                                                    if(name == Slice("fillseq100K")) {
                ...
                if(sscanf(argv[i], "--compression_ratio=%lf%c", &d, &junk) == 1) {
                    ...
                    if(sscanf(argv[i], "--use_existing_db=%d%c", &n, &junk) == 1 &&
                       (n == 0 || n == 1)) {
                        ...
                        if(sscanf(argv[i], "--use_rowids=%d%c", &n, &junk) == 1 &&
                           (n == 0 || n == 1)) {
                            FLAGS_use_rowids = n;
                        } else
                            if(sscanf(argv[i], "--num=%d%c", &n, &junk) == 1) {
                                ...
                                if(sscanf(argv[i], "--reads=%d%c", &n, &junk) == 1) {
                                    ...
                                    if(sscanf(argv[i], "--value_size=%d%c", &n, &junk) == 1) {
                                        ...
                                        if(leveldb::Slice(argv[i]) == leveldb::Slice("--no_transaction")) {
                                            FLAGS_transaction = false;
                                        } else
                                            if(sscanf(argv[i], "--page_size=%d%c", &n, &junk) == 1) {
                                                FLAGS_page_size = n;
                                            } else
                                                if(sscanf(argv[i], "--num_pages=%d%c", &n, &junk) == 1) {
                                                    FLAGS_num_pages = n;
                                                } else
                                                    if(sscanf(argv[i], "--WAL_enabled=%d%c", &n, &junk) == 1 &&
                                                       (n == 0 || n == 1)) {
                                                        FLAGS_WAL_enabled = n;
                                                    } else
